						...
						if (format["%1",_safepos] != "[-500,-500,0]" ) then
						{
							...
							if (_p_mcc_spawntype == "PARATROOPER") then
							{
									...
									if (_p_mcc_spawnfaction=="WEST") then
										{
										 nul=[[(_safepos select 0),(_safepos select 1),0],1,_p_mcc_spawnname,_p_mcc_zone_markername,_p_mcc_zone_behavior,_p_mcc_awareness] spawn (compile (preprocessFileLineNumbers( '\mcc_sandbox_mod\mcc\general_scripts\paratroops\paratroops.sqf')));
										 };	
									if (_p_mcc_spawnfaction=="EAST") then
										{
										 nul=[[(_safepos select 0),(_safepos select 1),0],2,_p_mcc_spawnname,_p_mcc_zone_markername,_p_mcc_zone_behavior,_p_mcc_awareness] spawn (compile (preprocessFileLineNumbers( '\mcc_sandbox_mod\mcc\general_scripts\paratroops\paratroops.sqf')));
										 };		
									if (_p_mcc_spawnfaction=="GUE") then
										{
										 nul=[[(_safepos select 0),(_safepos select 1),0],3,_p_mcc_spawnname,_p_mcc_zone_markername,_p_mcc_zone_behavior,_p_mcc_awareness] spawn (compile (preprocessFileLineNumbers( '\mcc_sandbox_mod\mcc\general_scripts\paratroops\paratroops.sqf')));
										 };		
							...
							if (_p_mcc_spawntype == "MAN") then
								{
									...
									_p_mcc_spawnname createUnit [_safepos, _unitspawned,"",MCC_AI_Skill select 0];
							...
							if (_p_mcc_spawntype == "GROUP") then
								
								{
											...
											if (_p_mcc_grouptype=="GUE") then
												{_unitspawned=[_safepos, resistance, (call compile _p_mcc_spawnfaction),[],[],MCC_AI_Skill select 1] call BIS_fnc_spawnGroup;};										
											if (_p_mcc_grouptype=="WEST") then
												{_unitspawned=[_safepos, west, (call compile _p_mcc_spawnfaction),[],[],MCC_AI_Skill select 1] call BIS_fnc_spawnGroup;};																				
											if (_p_mcc_grouptype=="EAST") then
												{_unitspawned=[_safepos, east, (call compile _p_mcc_spawnfaction),[],[],MCC_AI_Skill select 1] call BIS_fnc_spawnGroup;};										
											if (_p_mcc_grouptype=="CIV") then
												{_unitspawned=[_safepos, civilian, (call compile _p_mcc_spawnfaction),[],[],MCC_AI_Skill select 1] call BIS_fnc_spawnGroup;};										
