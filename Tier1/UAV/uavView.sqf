AW_fnc_establishingShot = {
	...
	*/
	...
	fakeUAV_tgt = [_this, 0, objNull, [objNull, []]] call BIS_fnc_param;
	...
	BIS_fnc_establishingShot_fakeUAV camPrepareTarget fakeUAV_tgt;
	BIS_fnc_establishingShot_fakeUAV camPreparePos _coords;
	BIS_fnc_establishingShot_fakeUAV camPrepareFOV fakeUAV_fov;
		...
		format [
			"
				...
				switch (_this select 1) do
				{
					...
					case 74:
					{
						fakeUAV_fov = fakeUAV_fov + 0.250;
						if (fakeUAV_fov > 1.700) then
						{
							fakeUAV_fov = 1.700;
						};
						BIS_fnc_establishingShot_fakeUAV camPrepareFOV fakeUAV_fov;
						BIS_fnc_establishingShot_fakeUAV camCommitPrepared 1;
						playSound 'click';
						true
					};
					...
					case 200:
					{
						fakeUAV_tgt = [(fakeUAV_tgt select 0), (fakeUAV_tgt select 1) + 25, 0];
						BIS_fnc_establishingShot_fakeUAV camPrepareTarget fakeUAV_tgt;
					...
					case 203:
					{
						fakeUAV_tgt = [(fakeUAV_tgt select 0) - 25, (fakeUAV_tgt select 1), 0];
						BIS_fnc_establishingShot_fakeUAV camPrepareTarget fakeUAV_tgt;
						BIS_fnc_establishingShot_fakeUAV camCommitPrepared 0.5;
					};
			...
			if (isNil "BIS_fnc_establishingShot_skip") then {
				...
				_key spawn {
					...
					if (!(isKeyActive _key) && isNil "BIS_fnc_establishingShot_skip") then {
						...
						private ["_skipText"];
						_skipText = format [
							"%1PRESS %2[SPACE]%3 TO EXIT%4",	// Todo: localize
							"<t size = '0.75'>",
							_keyColor,
							"</t>",
							"</t>"
...
[
	markerPos currentAO,
	format ["OPERATOR: %1|TARGET: %2||Press SPACE to EXIT|Press NUM 1 - 9 to switch viewing modes|Press NUM + and - to zoom|", name _obj, currentAO],
	300,
	230,
	random 360,
	1,
